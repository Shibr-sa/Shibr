...
export default function ShelfDetailsPage()
{
  ...
  // Fetch shelf data directly using the shelf ID
  const shelfData = useQuery(api.shelves.getShelfById,
    shelfId ? { shelfId: shelfId as Id<"shelves"> } : "skip"
  )
  ...
  // Fetch rental requests for this shelf
  ...
  if (shelfData === undefined || store === undefined)
  {
    return (
  ...
  const renterDetails = activeRental ? [{
    ...
    commercialRegister: activeRental.requesterProfile?.commercialRegisterNumber || activeRental.requesterProfile?.freelanceLicenseNumber || "-"
  }] : []

  // Format products data
  const allProducts = shelfProducts?.map(product => ({
    id: product?._id || "",
    name: product?.name || "",
    quantity: 'requestedQuantity' in product ? product.requestedQuantity : 0,
    sales: 'totalSales' in product ? product.totalSales || 0 : 0,
    price: product?.price || 0,
    imageUrl: 'imageUrl' in product ? product.imageUrl : undefined
  })) || []

  // Format payment records (placeholder for now)
  const allPaymentRecords: any[] = []

  // Format previous rentals
  ...
  const allPreviousInfo = previousRentals.map(rental =>
  {
    ...
    const durationInMs = rental.endDate - rental.startDate
    const durationInMonths = Math.ceil(durationInMs / (1000 * 60 * 60 * 24 * 30))
    ...
    return {
      ...
      rentalMethod: t("shelf_details.monthly_rental"),
      revenue: rental.totalAmount || (rental.monthlyPrice * durationInMonths)
    }
  })
                ...
                <div className="space-y-4">
                  ...
                  <div className="relative h-64 w-full">
                    ...
                    { formattedData.images.length > 1 && (
                      <Badge className="absolute top-3 start-3 bg-background/90 backdrop-blur-sm">
                        { formattedData.images.findIndex((img: string) => img === (selectedImage || formattedData.images[0])) + 1 } / { formattedData.images.length }
                      </Badge>
                    ) }
                  </div>
                  ...
                  { formattedData.images.length > 1 && (
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                      { formattedData.images.slice(0, 3).map((image: string, index: number) => (
                        <div
                          key={ index }
                        ...
                        >
                          <Image
                            ...
                            fill
                            className={ `object-cover rounded-md border-2 transition-colors ${(selectedImage === image || (!selectedImage && index === 0))
                ...
                <PaginationContent>
                  ...
                  <PaginationItem>
                    <PaginationNext
                      ...
                      className={ productsPage === totalProductPages ? "pointer-events-none opacity-50" : "cursor-pointer" }
                    />
                  </PaginationItem>
                </PaginationContent>
              </Pagination>
              ...
              <PaginationContent>
                ...
                <PaginationItem>
                  <PaginationNext
                    ...
                    className={ paymentsPage === totalPaymentPages ? "pointer-events-none opacity-50" : "cursor-pointer" }
                  />
                </PaginationItem>
              </PaginationContent>
            </Pagination>
          ...
          { totalPreviousPages > 1 && (
            <Pagination className="mt-4">
              <PaginationContent>
                ...
                { Array.from({ length: totalPreviousPages }, (_, i) => i + 1).map((page) => (
                  <PaginationItem key={ page }>
                    <PaginationLink
                ...
                <PaginationItem>
                  <PaginationNext
                    onClick={ () => setPreviousPage(prev => Math.min(totalPreviousPages, prev + 1)) }
                    className={ previousPage === totalPreviousPages ? "pointer-events-none opacity-50" : "cursor-pointer" }
                  />
                </PaginationItem>
              </PaginationContent>
            </Pagination>
