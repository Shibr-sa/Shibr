...
export default function ShelfDetailsPage()
{
  ...
  const formattedData = {
    ...
    city: shelfData.city,
    branch: shelfData.branchName,
    address: shelfData.location?.address,
  ...
  const allProducts: Array<{ id: string, name: string, quantity: number, sales: number, price: number, imageUrl?: string }> = shelfProducts?.map(product => ({
    id: product?._id || "",
    name: product?.name || "",
    quantity: product?.requestedQuantity || 0,
    sales: ('totalSales' in product && product.totalSales) ? product.totalSales : 0,
    price: product?.price || 0,
    imageUrl: ('imageUrl' in product && product.imageUrl) ? product.imageUrl : undefined,
  })) || []
  ...
  {
    // Calculate rental period in months from start and end dates
    const rentalPeriodMonths = rental.startDate && rental.endDate
      ? Math.ceil((rental.endDate - rental.startDate) / (1000 * 60 * 60 * 24 * 30))
      : 1
    ...
    return {
      ...
      revenue: rental.totalAmount || (rental.monthlyPrice * rentalPeriodMonths)
    }
  })
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div className="flex items-center gap-3 p-2 rounded-lg hover:bg-muted/50 transition-colors">
                    ...
                    <div className="flex-1 space-y-1 min-w-0">
                      ...
                      { shelfData.location ? (
                        <Link href={ `https://www.google.com/maps?q=${shelfData.location.lat},${shelfData.location.lng}` }
                          target="_blank"
                      ...
                      <TableCell className="py-3">
                        { renter.commercialRegisterUrl ? (
                          <Button
                            ...
                            onClick={ () =>
                            {
                              if (renter.commercialRegisterUrl)
                              {
