...
import { Search, FileText, Clock, CheckCircle2, DollarSign, ChevronLeft, ChevronRight, TrendingUp } from "lucide-react"
import Link from "next/link"
import { format } from "date-fns"
import { ar } from "date-fns/locale"

export default function StoreClearancesPage() {
  ...
  // Fetch clearances
  const data = useQuery(api.rentalClearance.getStoreClearances, {
    page: currentPage,
    limit: 10,
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          {t("clearances.store.description")}
        </p>
      </div>
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-muted-foreground">
                  {t("clearances.store.stats.pendingPayouts")}
                </p>
                <p className="text-2xl font-bold mt-1">{data.stats.pendingPayouts}</p>
              </div>
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-muted-foreground">
                  {t("clearances.store.stats.completedPayouts")}
                </p>
                <p className="text-2xl font-bold mt-1">{data.stats.completedPayouts}</p>
              </div>
        ...
        <CardContent className="p-6">
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <div className="relative flex-1 max-w-sm">
              ...
              <Input
                placeholder={t("clearances.store.searchPlaceholder")}
                value={searchQuery}
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className={direction === "rtl" ? "text-right" : ""}>
                    {t("clearances.store.table.brand")}
                  </TableHead>
                  <TableHead className={direction === "rtl" ? "text-right" : ""}>
                  ...
                  <TableHead className={direction === "rtl" ? "text-left" : "text-right"}>
                    {t("clearances.store.table.payout")}
                  </TableHead>
                  <TableHead className={direction === "rtl" ? "text-left" : "text-right"}>
                    ...
                    <TableRow key={item._id}>
                      ...
                      <TableCell className={direction === "rtl" ? "text-left" : "text-right"}>
                        <Button
                          ...
                          asChild
                        >
                          <Link href={`/store-dashboard/clearances/${item.clearanceId}`}>
